...
public final class LegacyTacletMatcher implements TacletMatcher {
        ...
        for (int j = 0, arity = term.arity(); j < arity && matchCond != null; j++) {
            ...
            if (bound.size() == tplBound.size()) {
                for (int i = 0, boundSize = bound.size(); i < boundSize && matchCond != null; i++) {
                    ...
                    matchCond = ElementMatcher.getElementMatcherFor(templateQVar)
                            .match(templateQVar, qVar, matchCond, services);
                }
            } else {
                matchCond = null;
            }
    ...
    /**
     * returns the matchconditions that are required if the java block of the given term matches the
    ...
    /**
     * returns a SVInstantiations object with the needed SchemaVariable to Term mappings to match
    ...
    private MatchConditions match(final Term term, final Term template, MatchConditions matchCond,
    ...
    private Pair<Term, MatchConditions> matchAndIgnoreUpdatePrefix(final Term term,
        ...
        if (sourceOp instanceof UpdateApplication && !(templateOp instanceof UpdateApplication)) {
