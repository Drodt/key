 ...
 * tree. The requirement is that the proof contains a predicate which is not evaluable to filter
 * invalid execution tree paths. The easiest way to achieve this is to use a
 * {@link FunctionalOperationContractPO} (addUninterpretedPredicate = {@code true}) instance as
 * proof obligation to instantiate a {@link Proof} from.
 * </p>
 * <p>
 * A symbolic execution tree consists of {@link IExecutionNode}s which represents the executed
 * statements and other Java constructs. The root of a symbolic execution tree is an
 ...
 * </p>
 * <p>
 * The symbolic execution tree is not updated automatically when KeY's proof tree has changed. The
 * update must be started manually via {@link #analyse()}. In this case the proof tree will be
 * analyzed and the execution tree model created or updated if it already exist.
 * </p>
 * <p>
 * Proof trees and also symbolic execution trees are very large even in small programs. For this
 * reason it is not possible to iterate over the tree via recursive method calls. Instead, an
 * instance of {@link ExecutionNodePreorderIterator} should be used to iterate over a symbolic
 * execution tree.
 * </p>
 * <p>
...
public class SymbolicExecutionTreeBuilder {
    ...
    /**
     * <p>
     * Maps a {@link Node} of KeY's proof tree to his execution tree model representation if it is
     * available.
     * </p>
     * <p>
     * In case that the {@link Node} is represented by multiple {@link AbstractExecutionNode}s, e.g.
    ...
    public SymbolicExecutionTreeBuilder(Proof proof, boolean mergeBranchConditions,
        ...
        assert proof != null;
        this.proof = proof;
        this.isUninterpretedPredicateUsed =
            AbstractOperationPO.getUninterpretedPredicate(getProof()) != null;
    ...
    protected void initMethodCallStack(final Node root, Services services) {
        ...
        if (modalityTerms.isEmpty()) {
            throw new IllegalStateException(
                "Sequent contains no modalities with symbolic execution label.");
        }
        ...
        if (label == null) {
            throw new IllegalStateException(
                "Modality \"" + modalityTerm + "\" has no symbolic execution term label.");
        }
    ...
    protected Set<Node> getMethodReturnsToIgnore(int id) {
        synchronized (methodReturnsToIgnoreMap) {
            Integer key = id;
            Set<Node> result =
                methodReturnsToIgnoreMap.computeIfAbsent(key, k -> new LinkedHashSet<>());
            return result;
        }
    }
    ...
    protected Map<Node, ImmutableList<Node>> getMethodCallStack(int id) {
        synchronized (methodCallStackMap) {
            ...
            Map<Node, ImmutableList<Node>> result =
                methodCallStackMap.computeIfAbsent(key, k -> new HashMap<>());
            return result;
        }
    }

    /**
     * Frees all resources. If this method is called it is no longer possible to use the
     * {@link SymbolicExecutionTreeBuilder} instance! Later usage is not checked and punished with
     * exceptions.
     */
    public void dispose() {
    ...
    /**
     * This method must be called programmatically to update the symbolic execution tree. The first
     * call will create the initial tree and further call will update the existing tree.
     *
    ...
    /**
     * Instances of this class are returned by {@link SymbolicExecutionTreeBuilder#analyse()} to
     * inform about newly completed blocks and returned methods.
     *
     * @author Martin Hentschel
     */
    ...
    /**
     * This {@link ProofVisitor} is used to transfer the proof tree in KeY into
     * {@link IExecutionNode}s which forms the symbolic execution tree.
     *
     * @author Martin Hentschel
     */
    private class AnalyzerProofVisitor implements ProofVisitor {
        ...
        public void visit(Proof proof, Node visitedNode) {
            // Find the parent node (IExecutionNode) to that the execution tree model representation
            // of the given KeY's proof tree node should be added.
                ...
                while (iter.hasNext()) {
                    ...
                    if (!keyNodeBranchConditionMapping.containsKey(childNode)) {
                        if (!shouldPrune(visitedNode)) { // Filter out branches that are closed
                            ...
                            ExecutionBranchCondition condition = new ExecutionBranchCondition(
                                settings, childNode, additionalBranchLabel);
                            // Add branch condition to the branch condition attributes for later
                            // adding to the proof tree. This is required for instance to filter out
                            // branches after the symbolic execution has finished.
                            ...
                            if (SymbolicExecutionUtil
                                    .hasSymbolicExecutionLabel(visitedNode.getAppliedRuleApp())) {
        ...
        /**
         * Instantiates all missing {@link IExecutionLink}s and injects them into the symbolic
         * execution tree.
         */
        public void injectLinks() {
            for (Node node : joinNodes) {
                MergeRuleBuiltInRuleApp ruleApp =
                    (MergeRuleBuiltInRuleApp) node.getAppliedRuleApp();
        ...
        /**
         * <p>
         * Completes the execution tree model after all {@link Node}s were visited in
         * {@link #visit(Proof, Node)}. The task of this method is to add
        ...
        public void completeTree() {
            Iterator<Entry<AbstractExecutionNode<?>, List<ExecutionBranchCondition>>> stackIter =
                branchConditionsStack.iterator();
            while (stackIter.hasNext()) {
                Entry<AbstractExecutionNode<?>, List<ExecutionBranchCondition>> entry =
                    stackIter.next();
    ...
    /**
     * <p>
     * Analyzes the given {@link Proof} and his contained proof tree by filling the start node
     * {@link SymbolicExecutionTreeBuilder#getStartNode()} with {@link IExecutionNode}s which are
     * instantiated if a {@link Node} in KeY's proof tree applies a rule of symbolic execution.
     * </p>
     * <p>
     * <b>Attention :</b> A correct pruning requires at the moment that the Taclet Option
     * "runtimeExceptions" is set to "runtimeExceptions:allow". Alternatively it is required to
     * modify rule {@code assignment_to_reference_array_component} in file {@code javaRules.key} by
     * uncommenting
        ...
        if (!shouldPrune(node)) { // Prune closed branches because they are invalid
            ...
            if (SymbolicExecutionUtil.isSymbolicExecutionTreeNode(node,
                ...
                Map<JavaPair, ImmutableList<IExecutionNode<?>>> completedBlocks =
                    updateAfterBlockMap(node, currentOrFutureRuleApplication);
                if (completedBlocks != null) {
                    for (Entry<JavaPair, ImmutableList<IExecutionNode<?>>> entry : completedBlocks
                            .entrySet()) {
            ...
            if (executionNode == null) {
                ...
                executionNode =
                    createExecutionTreeModelRepresentation(parentToAddTo, node, statement);
            ...
            // Check if loop condition is available after loop invariant (rewritten into if
            // statement)
            if (statement instanceof If
                    && ((If) statement).getExpression().getPositionInfo() != PositionInfo.UNDEFINED
                    && SymbolicExecutionUtil
                            .getSymbolicExecutionLabel(node.getAppliedRuleApp()) != null
    ...
    protected IExecutionBranchCondition searchDirectParentBodyPreservesInvariantBranchCondition(
        ...
        while (current instanceof IExecutionBranchCondition) {
            if (WhileInvariantRule.BODY_PRESERVES_INVARIANT_LABEL
                    .equals(current.getProofNode().getNodeInfo().getBranchLabel())) {
    ...
    protected AbstractExecutionNode<?> createExecutionTreeModelRepresentation(
        ...
        if (SymbolicExecutionUtil.hasSymbolicExecutionLabel(node.getAppliedRuleApp())) {
            if (statement != null
                    && !SymbolicExecutionUtil.isRuleAppToIgnore(node.getAppliedRuleApp())) {
                // Get position information
                PositionInfo posInfo = statement.getPositionInfo();
                // Determine the node representation and create it if one is available
                if (SymbolicExecutionUtil.isMethodCallNode(node, node.getAppliedRuleApp(),
                    statement)) {
                ...
                } else if (SymbolicExecutionUtil.isBranchStatement(node, node.getAppliedRuleApp(),
                    statement, posInfo)) {
                ...
                } else if (SymbolicExecutionUtil.isLoopStatement(node, node.getAppliedRuleApp(),
                    statement, posInfo)) {
                    if (isNotInImplicitMethod(node)) {
            ...
            } else if (SymbolicExecutionUtil.isBlockSpecificationElement(node,
                node.getAppliedRuleApp())) {
                if (isNotInImplicitMethod(node)) {
            ...
            result = new ExecutionTermination(settings, node, exceptionVariable,
                TerminationKind.LOOP_BODY);
            startNode.addTermination((ExecutionTermination) result);
        }
        return result;
    }
    ...
    protected Map<Node, Map<JavaPair, ImmutableList<IExecutionNode<?>>>> getAfterBlockMaps(int id) {
        synchronized (afterBlockMap) {
            ...
            Map<Node, Map<JavaPair, ImmutableList<IExecutionNode<?>>>> result =
                afterBlockMap.computeIfAbsent(key, k -> new LinkedHashMap<>());
            return result;
        }
    }
            ...
            while (!done && expectedStatementsIterator.hasNext()) {
                ...
                if (SymbolicExecutionUtil.equalsWithPosition(next, currentActiveStatement)) {
                        ...
                        && (currentInnerMostMethodFrame != null
                                && currentInnerMostMethodFrame.getBody().isEmpty()
                                || (next != null && !SymbolicExecutionUtil.containsStatement(
                                    currentInnerMostMethodFrame, next, proof.getServices())))) {
                    done = true;
                }
            }
        }
        ...
        if (SymbolicExecutionUtil.hasSymbolicExecutionLabel(node.getAppliedRuleApp())) {
            if (statement != null
                ...
                boolean methodReturn =
                    SymbolicExecutionUtil.isMethodReturnNode(node, node.getAppliedRuleApp());
                boolean exceptionalMethodReturn = !methodReturn && SymbolicExecutionUtil
                        .isExceptionalMethodReturnNode(node, node.getAppliedRuleApp());
                if (methodReturn || exceptionalMethodReturn) {
                    ...
                    if (callNode != null) {
                        ...
                        Set<Node> methodReturnsToIgnore =
                            getMethodReturnsToIgnore(node.getAppliedRuleApp());
                        if (!methodReturnsToIgnore.contains(callNode)) {
                            ...
                            if (callSEDNode instanceof ExecutionMethodCall) { // Could be the start
                                ...
                                if (methodReturn) {
                                    ...
                                    result = new ExecutionExceptionalMethodReturn(settings, node,
                                        (ExecutionMethodCall) callSEDNode);
                                    completions.addMethodReturn(result);
                                }
                            }
    ...
    /**
     * This method initializes the method call stack of loop body modalities with the values from
    ...
    /**
     * This method initializes the method call stack of validity modalities with the values from the
     ...
     * {@link SymbolicExecutionTreeBuilder#initNewLoopBodyMethodCallStack(Node)} to compute the
     * number of available {@link MethodFrame}s.
     *
     * @author Martin Hentschel
     */
    ...
    /**
     * <p>
     * Searches the {@link IExecutionNode} which represents the given {@link Node} of KeY's proof
     * tree.
     * <p>
     * In case that the {@link Node} is represented by multiple {@link AbstractExecutionNode}s, e.g.
     * a return statement and a method return, the last node is returned.
    ...
    /**
     * Returns the minimal required PO {@link Properties} to support symbolic execution tree
     * extraction in a {@link SymbolicExecutionJavaProfile}.
     *
