...
import org.key_project.rusty.strategy.termgenerator.MultiplesModEquationsGenerator;
import org.key_project.rusty.strategy.termgenerator.RootsGenerator;
...
public class IntegerStrategy extends AbstractFeatureStrategy implements ComponentStrategy {
    ...
    public IntegerStrategy(Proof proof, StrategyProperties strategyProperties) {
        super(proof);
        this.tf = new ArithTermFeatures(proof.getServices().getLDTs().getIntLDT());
    ...
    private RuleSetDispatchFeature setupApprovalDispatcher() {
        final RuleSetDispatchFeature d = new RuleSetDispatchFeature();
        final IntLDT numbers = getServices().getLDTs().getIntLDT();
