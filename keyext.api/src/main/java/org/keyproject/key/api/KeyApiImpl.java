...
import org.key_project.rusty.pp.PosTableLayouter;
import org.key_project.rusty.pp.PositionTable;
...
import static org.key_project.rusty.proof.ProofNodeDescription.collectPathInformation;
...
public final class KeyApiImpl implements KeyApi {
    ...
    public CompletableFuture<Either<EnvironmentId, ProofId>> load(LoadParams params) {
        return CompletableFutures.computeAsync((c) -> {
            ...
            KeYEnvironment<?> env;
            try {
                var file = new File(params.problemFile().asPath().toString());
